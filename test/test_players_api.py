...
class TestPlayersApi(MainTest):
    ...
    def test_delete(self):
        ...
        """
        for status, json in self.load_json('players', 'delete'):
            responses.reset()
    ...
    def test_delete_logo(self):
        ...
        """
        for status, json in self.load_json('players', 'delete_logo'):
            responses.reset()

            kwargs = {
                'player_id': "pl14Db6oMJRH6SRVoOwORacK",
            }
            url = '/players/{player_id}/logo'.format(**kwargs)

            responses.add('DELETE', url, body=json, status=int(status), content_type='application/json')

            if status[0] == '4':
                with self.assertRaises(ApiException) as context:
                    self.api.delete_logo(**kwargs)
                if status == '404':
                    self.assertIsInstance(context.exception, NotFoundException)
            else:
                self.api.delete_logo(**kwargs)
    ...
    def test_upload_logo(self):
        ...
        """
        for status, json in self.load_json('players', 'upload_logo'):
            responses.reset()
